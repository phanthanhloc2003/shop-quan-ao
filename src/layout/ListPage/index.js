...
function Listpage(props) {
    ...
    const filteredData = productList.filter((item) => {
        return ((categoryId === 0 || item.category.id === categoryId) && ((priceProduct.priceStart === 0 || item.salePrice >= priceProduct.priceStart) && (priceProduct.priceEnd === 0 || item.salePrice <= priceProduct.priceEnd)));
    })
    ...
    const currentData = filteredData.slice((pageIndex - 1) * limit, pageIndex * limit);
                ...
                <Grid container spacing={1}>
                    <Grid className="w-[250px]">
                        <Paper className="mr-[4px]" elevation={0}>
                            <ProductFilters filters={categoryId} onChange={handleFiltersChange} />
